...
public class Render {
	...
	private Color getColorAtPoint(Geometry geometry, Point3D point) {
		...
		// calculate the total colors of the red, green, and blue light
...
//		if(numberOfLights != 0) {
			...
			while(lights.hasNext()) {
				if(!occluded(lights, point, geometry)) {
	...
	// occluded checks if there are intersection between the geometries (plane and triangle)
	//If yes, then there are shadow rays
	public boolean occluded(LightSource light, Point3D point, Geometry geometry) {
		// light direction from the intersecting object
		...
		epsVector.scale(2);
		// add the 3d geometry point to the epsVector
		geometryPoint.add(epsVector);
		// declare a ray of light (lightRay) from the point of intersection and the direction of the light
		Ray lightRay = new Ray(geometryPoint, lightDirection);
		//map of all the points the rya of light intersects with out larger object from the list of points and the shape
		Map<Geometry, List<Point3D>> intersectionPoints = getSceneRayIntersections(lightRay);
		// if the shape is a flat geometry, then remove the shape from the larger object, so that a shadow is seen.
		if (geometry instanceof FlatGeometry){
			intersectionPoints.remove(geometry);
		}
		// return true if the map of intersection points is not empty, false if it is
